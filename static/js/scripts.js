...
function initializeFunctions() {
    ...
    if (document.querySelector('.search-toggle')) {initializeSearch();}
...
function handleAddExpenseForm() {
        ...
        if (addSplitTypeSelect.value === 'custom') {
            ...
            checkboxes.forEach(checkbox => {
                ...
                if (input) {
                    ...
                    if (isNaN(amount) || amount <= 0 || !isValidTwoDecimalPlaces(amount)) {
                        event.preventDefault();
                        addErrorMessage.textContent = 'Please enter valid amounts (up to two decimal places) for all participants.';
                        return;
                    }
                    totalAmount = roundToTwoDecimals(totalAmount + amount);
                    participantsData[checkbox.value] = amount;
                }
            });
            const totalExpenseAmount = parseFloat(document.getElementById('amount').value);
            if (!isValidTwoDecimalPlaces(totalExpenseAmount)) {
                event.preventDefault();
                addErrorMessage.textContent = 'Total expense amount should have up to two decimal places.';
                return;
            }
            if (Math.abs(totalAmount - totalExpenseAmount) > 0.005) { 
                event.preventDefault();
    ...
    function isValidTwoDecimalPlaces(value) {
...
function initializeSearch() {
    ...
    if (searchToggle && searchContainer && searchForm && searchInput && searchIcon) {
        let isSearchActive = false;

        function toggleSearchIcon() {
            if (isSearchActive) {
                searchIcon.classList.remove('fa-magnifying-glass-plus');
                searchIcon.classList.add('fa-magnifying-glass-minus');
            } else {
                searchIcon.classList.remove('fa-magnifying-glass-minus');
                searchIcon.classList.add('fa-magnifying-glass-plus');
            }
        }
        ...
        if (splitType === 'custom') {
            ...
            checkboxes.forEach(checkbox => {
                ...
                if (input) {
                    const amount = parseFloat(input.value);
                    if (isNaN(amount) || amount <= 0 || !isValidTwoDecimalPlaces(amount)) {
                        editErrorMessage.textContent = 'Please enter valid amounts (up to two decimal places) for all participants.';
                        return;
                    }
                    totalAmount = roundToTwoDecimals(totalAmount + amount);
                    participantsData[checkbox.value] = amount;
        ...
        xhr.onload = function () {
            if (xhr.status === 200) {
                try {
                    ...
                    if (response.personal_expense) {
                        var separatorDiv = document.createElement('div');
                        separatorDiv.className = 'separator';
                        debtsResultDiv.appendChild(separatorDiv);

                        var expenseDiv = document.createElement('div');
                        expenseDiv.className = 'personal_expense';
                        expenseDiv.textContent = `${response.personal_expense.username}'s expense: ${response.personal_expense.amount} ${response.personal_expense.currency}`;
                        debtsResultDiv.appendChild(expenseDiv);
                    }
